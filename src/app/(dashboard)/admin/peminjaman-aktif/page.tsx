import { createClient } from '@/lib/supabase/server'
import { redirect } from 'next/navigation'
import { Clock, CheckCircle2, User, MapPin } from 'lucide-react'

export const dynamic = 'force-dynamic'

export default async function ActiveBorrowingsOverview() {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')

  const { data: profile } = await supabase
    .from('profiles')
    .select('*')
    .eq('id', user.id)
    .single()

  if (profile?.role !== 'admin') redirect('/dashboard')

  // Fetch active tool borrowings
  const { data: tools } = await supabase
    .from('tool_borrowings')
    .select(`
      *,
      profiles (full_name, nim),
      inventory (name, location)
    `)
    .eq('status', 'approved')
    .order('start_date', { ascending: false })

  // Fetch active room bookings
  const { data: rooms } = await supabase
    .from('room_bookings')
    .select(`
      *,
      profiles (full_name, nim),
      rooms (name, location)
    `)
    .eq('status', 'approved')
    .order('start_time', { ascending: false })

  return (
    <div className="page-content">
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.25rem' }}>
          Laporan Peminjaman Aktif
        </h1>
        <p style={{ color: 'var(--text-muted)' }}>
          Ikhtisar seluruh alat dan ruangan yang sedang digunakan (Approved) saat ini.
        </p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        
        {/* Active Tools Section */}
        <section className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)' }}>Alat yang Sedang Dipinjam</h2>
            <div style={{ fontSize: '0.875rem', fontWeight: 600, backgroundColor: 'var(--accent-glow)', color: 'var(--accent-primary)', padding: '0.25rem 0.75rem', borderRadius: '1rem' }}>
              {tools?.length || 0} Aktif
            </div>
          </div>

          {!tools || tools.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--text-muted)' }}>
              <CheckCircle2 size={48} style={{ margin: '0 auto 1rem auto', opacity: 0.2 }} />
              <p>Tidak ada peminjaman alat yang aktif saat ini.</p>
            </div>
          ) : (
            <div className="grid-3">
              {tools.map((t: any) => (
                <div key={t.id} style={{ padding: '1.25rem', border: '1px solid var(--border)', borderRadius: '0.5rem', background: 'var(--bg-secondary)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem', fontSize: '1.05rem' }}>
                    {t.inventory?.name}
                  </div>
                  <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <User size={14} />
                      <span>{t.profiles?.full_name} ({t.profiles?.nim})</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <MapPin size={14} />
                      <span>{t.inventory?.location || 'Lokasi tidak diset'} - Jumlah: {t.quantity}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Clock size={14} />
                      <span>{new Date(t.start_date).toLocaleDateString()} s/d {new Date(t.end_date).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <div style={{ fontSize: '0.8rem', padding: '0.5rem', background: 'var(--bg-primary)', borderRadius: '0.25rem', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
                    <strong>Tujuan:</strong> {t.purpose || '-'}
                  </div>
                </div>
              ))}
            </div>
          )}
        </section>

        {/* Active Rooms Section */}
        <section className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)' }}>Ruangan yang Sedang Digunakan</h2>
            <div style={{ fontSize: '0.875rem', fontWeight: 600, backgroundColor: 'var(--accent-glow)', color: 'var(--accent-primary)', padding: '0.25rem 0.75rem', borderRadius: '1rem' }}>
              {rooms?.length || 0} Aktif
            </div>
          </div>

          {!rooms || rooms.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--text-muted)' }}>
              <CheckCircle2 size={48} style={{ margin: '0 auto 1rem auto', opacity: 0.2 }} />
              <p>Tidak ada ruangan yang aktif digunakan saat ini.</p>
            </div>
          ) : (
            <div className="grid-3">
              {rooms.map((r: any) => (
                <div key={r.id} style={{ padding: '1.25rem', border: '1px solid var(--border)', borderRadius: '0.5rem', background: 'var(--bg-secondary)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem', fontSize: '1.05rem' }}>
                    {r.rooms?.name}
                  </div>
                  <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <User size={14} />
                      <span>{r.profiles?.full_name} ({r.profiles?.nim})</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <MapPin size={14} />
                      <span>{r.rooms?.location || 'Lokasi tidak diset'}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Clock size={14} />
                      <span>{new Date(r.start_time).toLocaleString()} - {new Date(r.end_time).toLocaleTimeString()}</span>
                    </div>
                  </div>
                  <div style={{ fontSize: '0.8rem', padding: '0.5rem', background: 'var(--bg-primary)', borderRadius: '0.25rem', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
                    <strong>Tujuan:</strong> {r.purpose || '-'}
                  </div>
                </div>
              ))}
            </div>
          )}
        </section>

      </div>
    </div>
  )
}
